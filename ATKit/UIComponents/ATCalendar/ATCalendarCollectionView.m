...
@interface ATCalendarCollectionView()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
...
@implementation ATCalendarCollectionView
...
- (instancetype)initWithFrame:(CGRect)frame collectionViewLayout:(nonnull UICollectionViewLayout *)layout
{
    if (self = [super initWithFrame:frame collectionViewLayout:layout]) {
        ...
        [self registerClass:[ATCalendarCell class] forCellWithReuseIdentifier:identifier];
    }
    return self;
}
...
{
    ATCalendarCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:identifier forIndexPath:indexPath];
    cell.backgroundColor = [UIColor whiteColor];
    
    if(indexPath.section == 0) {
        cell.weekDay = self.titles[indexPath.row];
    } else {
        ATCalendarDate *date = self.dates[indexPath.row];
        cell.date = date;
        
        /** 当前显示的日期 */
        NSString *dateString;
        if (date.isLastMonth) { // 上个月
            dateString = [[ATCalendarHelper lastYearAndMonth:self.yearAndMonth] stringByAppendingFormat:@"-%02zd",date.date.integerValue];
        } else if (date.isNextMonth) { // 下个月
            dateString = [[ATCalendarHelper nextYearAndMonth:self.yearAndMonth] stringByAppendingFormat:@"-%02zd",date.date.integerValue];
        } else { // 这个月
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    if ([self.collectionViewDelegate respondsToSelector:@selector(calendarView:didSelectItem:date:dateString:)]) {
        ...
        NSDate *dateObj = [ATCalendarHelper dateStringToDate:dateString format:@"yyyy-MM-dd"];
...
- (void)setYearAndMonth:(NSString *)yearAndMonth
{
    ...
    if (yearAndMonth) {
        _dates = [ATCalendarHelper datesWithYearAndMonth:yearAndMonth];
        [self reloadData];
    }
}
