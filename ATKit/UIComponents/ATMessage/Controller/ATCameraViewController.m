...
#import "ATAVPlayer.h"
#import "ATProgressView.h"

typedef void(^PropertyChangeBlock)(AVCaptureDevice *captureDevice);
@interface ATCameraViewController () <AVCaptureFileOutputRecordingDelegate>
...
//视频播放
@property (strong, nonatomic) ATAVPlayer *player;

@property (strong, nonatomic) IBOutlet ATProgressView *progressView;
...
@property (copy, nonatomic) NSString *takeImagePath;
...
@implementation ATCameraViewController
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    if ([[touches anyObject] view] == self.imgRecord) {
        ...
        if (![self.captureMovieFileOutput isRecording]) {
            ...
            NSString *outputFielPath = [[ATMediaManager sharedInstance] getFullCacheDirectoryWithMediaType:ATMediaType_video];
    ...
    if (self.saveVideoUrl) {
        ...
        [assetsLibrary writeVideoAtPathToSavedPhotosAlbum:self.saveVideoUrl completionBlock:^(NSURL *assetURL, NSError *error) {
            ...
            if (error) {
                ...
                [ATTips tipsWithMessage:@"保存视频到相册发生错误"];
            } else {
                if (weakSelf.takeBlock) {
                    weakSelf.takeBlock(self.saveVideoUrl);
                }
                NSLog(@"成功保存视频到相簿.");
        ...
        if (self.takeBlock) {
            self.takeBlock(self.takeImagePath);
        }
...
-(void)captureOutput:(AVCaptureFileOutput *)captureOutput didStartRecordingToOutputFileAtURL:(NSURL *)fileURL fromConnections:(NSArray *)connections{
    ...
    [self performSelector:@selector(onStartTranscribe:) withObject:fileURL afterDelay:.5];
}
...
-(void)captureOutput:(AVCaptureFileOutput *)captureOutput didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL fromConnections:(NSArray *)connections error:(NSError *)error{
    ...
    if (self.isVideo) {
        ...
        if (!self.player) {
            self.player = [[ATAVPlayer alloc] initWithFrame:self.bgView.bounds withShowInView:self.bgView url:outputFileURL animated:NO];
        } else {
            if (outputFileURL) {
...
- (void)videoHandlePhoto:(NSURL *)url {
    ...
    self.takeImagePath = [[ATMediaManager sharedInstance] saveImageToLocalSandBox:image];
